...
def Cloud(p, a, b, nvec):
    ...
    ## Neighbor search.
    vec = find_neighbors(p, dist, a, b, nvec)
...
def min_distances(p, mode = 2):
    ...
    Output:
        dist            float           The minimum distance between two consecutive nodes.
    """
        ...
        for i in np.arange(m):                                                      # For each of the nodes.
            ...
            for j in np.arange(m):                                                  # For all the nodes.
                if i != j:                                                          # If the the node is different to the central one.
                    ...
                    d  = np.sqrt((x - x1)**2 + (y - y1)**2)                         # Distance from the possible neighbor to the central node.
                    dmin[i] = min(dmin[i], d)                                       # Look for the distance to the closest node.
        dist = np.min(dmin)                                                         # The distance is the minimum distance between two consecutive nodes.
...
def find_neighbors(p, dist, a, b, nvec, n_jobs = -1):
    """
    find_neighbors
