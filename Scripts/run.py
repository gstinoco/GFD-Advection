...
from Scripts.Neighbors import min_distances
import Advection
...
def process_region(f, a, b, t, region, files, data_path, results_path, save):
    ...
    if '_p.csv' in files and '_tt.csv' in files:                                            # Check the existence of points and triangles.
        ...
        ## Calculate the minimum distance between the nodes
        c1   = (max(p[:,0]) - min(p[:,0]))/2
        c2   = (max(p[:,1]) - min(p[:,1]))/2

        u_ap, u_ex, vec = Advection.Cloud(p, f, a, b, c1, c2, t, triangulation = triangulation, tt = tt, implicit = Implicit, lam = 0.1)
...
def run_simulation(f, a, b, t, implicit, data, exam = 'test', holes = False, save = True):
    ...
    for region, files in regions_c.items():                                                 # For each of the regions.
        process_region(f, a, b, t, region, files, data, results_clouds, Save)               # Process the region.