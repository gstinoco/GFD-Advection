...
def Cloud(p, f, a, b, c1, c2, t, LW = True, triangulation = False, tt = [], implicit = False, lam = 0.5):
    """
    ...
    for k in np.arange(t):                                                          # For each time step.
        u_ap[boun_n, k] = f(p[boun_n, 0], p[boun_n, 1], T[k], a, b, c1, c2)         # The boundary condition is assigned.
  
    # Initial condition
    u_ap[:, 0] = f(p[:, 0], p[:, 1], T[0], a, b, c1, c2)                            # The initial condition is assigned.
    ...
    if triangulation == True:                                                       # If there are triangles available.
        ...
        vec = Neighbors.Cloud(p, a, b, nvec)                                        # Neighbor search with the proper routine.

    # Computation of Gamma values
    if LW == True:                                                                  # Prefer the Lax-Wendorff modified scheme.
        L = np.vstack([[-a*dt], [-b*dt], [(a*dt)**2], [(a*b*dt)**2], [(b*dt)**2]])  # The values of the differential operator are assigned.
    else:                                                                           # Use the classical GFD scheme.
    ...
    for k in np.arange(t):                                                          # For all the time steps.
        u_ex[:, k] = f(p[:, 0], p[:, 1], T[k], a, b, c1, c2)                        # The theoretical solution is computed.
